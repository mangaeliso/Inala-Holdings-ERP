...
import { getTransactions, getLoans } from '../services/firestore';
...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Mock fetching data for "t_biz_01" (Meat Business) for demo purposes
      const txs = await getTransactions('t_biz_01');
      ...
      const data = last7Days.map(date => {
        const dayTotal = txs
          .filter(t => {
              if (typeof t.timestamp !== 'string') return false; // Safety check
              return t.timestamp.startsWith(date);
          })
          .reduce((acc, t) => acc + t.amount, 0);
        return { name: new Date(date).getDate().toString(), value: dayTotal };
      });
      setChartData(data);

      // Mock loans
      const loans = await getLoans('t_loan_01');
      const totalLoans = loans.reduce((acc, l) => acc + l.amount, 0);
      setLoanTotal(totalLoans);

      // Simulating a delay for AI
      setTimeout(() => {
        ...
        <Card className="hover:translate-y-[-2px] transition-transform duration-300">
           <div className="flex justify-between items-start">
             <div>
               ...
               <h3 className="text-3xl font-bold mt-2">R {revenue.toLocaleString()}</h3>
             </div>
        ...
        <Card className="hover:translate-y-[-2px] transition-transform duration-300">
           <div className="flex justify-between items-start">
             <div>
               ...
               <h3 className="text-3xl font-bold mt-2">R {loanTotal.toLocaleString()}</h3>
             </div>
      ...
      <Card className="p-6">
          ...
          <div className="w-full h-[300px] min-w-0">
            {chartData.length > 0 ? (
                ...
                <div className="h-full flex items-center justify-center text-slate-400 text-sm">Loading chart data...</div>
            )}
          </div>
      </Card>
    </div>
